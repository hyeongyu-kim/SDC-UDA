...
class BraTS2018(Cityscapes):
        
    def __init__(self,
                 list_path='/home/compu/HG/CrossModa/DRANet-master/DRANet-master/data_list/BraTS2018/',
                 contrast='t1',
                 split='train',
                 crop_size=(256, 256),
                 train=True,
    ...
    def __getitem__(self, item):
        ...
        image = torch.tensor(sitk.GetArrayFromImage(sitk.ReadImage(image_path)).astype(np.float32))

        gt_image_path = image_path.replace(self.contrast,'seg')
        gt_image = torch.tensor(sitk.GetArrayFromImage(sitk.ReadImage(gt_image_path)).astype(np.float32))
        
        
        if (self.split == "train" or self.split == "trainval" or self.split =="all") and self.train:
            slices_z_all = image.shape[0]
            z_chosen = random.randint(int(slices_z_all*0.2) , int(slices_z_all*0.8) )

            image = image[z_chosen][None,:]
            gt_image = gt_image[z_chosen][None,:]
