...
label_colours_SYNAPSE = list(map(tuple, label_colours_SYNAPSE))

def decode_labels_SYNAPSE(imgs, mask, num_images=1, num_classes=3):
        ...
        for j_, j in enumerate(mask[i, :, :]):
            for k_, k in enumerate(j):
                if k < num_classes and k> 0:
                    img[int(j_), int(k_)] = label_colours_SYNAPSE[int(k)]
...
class SYNAPSE_3d(Cityscapes):
        
    def __init__(self,
                 list_path='/home/compu/HG/CrossModa/DRANet-master/DRANet-master/data_list/SYNAPSE/',
                 contrast='t1',
                 split='train',
                 crop_size=(256, 256),
                 train=True,
        ...
        self.id_to_trainid = {0: 0, 1: 1, 2: 2, 3:3,6:4}
    ...
    def __getitem__(self, item):
        ...
        # pid = image_path.split('/')[-1]
        # if self.contrast =='CT':
        #     gt_image_path = pid.split('_')[0] + '_label_' + pid.split('_')[1].replace('img','label')
        # else : 
        #     gt_image_path = image_path.replace('image' ,'image_label').replace('img','label')
...
class SYNAPSE_2d(Cityscapes):
        
    def __init__(self,
                 list_path='/home/compu/HG/CrossModa/DRANet-master/DRANet-master/data_list/SYNAPSE_2d/',
                 contrast='mr',
                 split='train',
                 crop_size=(256, 256),
                 train=True,
...
class SYNAPSE_25d(Cityscapes):
    ...
    def __getitem__(self, item):
        ...
        image= np.concatenate([image_0, image_1, image_2], axis=-1).astype(np.float)
        ...
        # gt_image= np.concatenate([gt_image0, gt_image1, gt_image2], axis=-1).astype(np.float)
        # gt_image=  torch.tensor(gt_image)
        # # gt_image = torch.tensor(gt_image)[:,:,None]
        # # gt_image = gt_image[:,:, z_choice-int((self.in_ch-1)/2) : z_choice+(int(self.in_ch/2)+1)]
...
class SYNAPSE_25d_Nd(Cityscapes):
...
class SYNAPSE_25d_Nd(Cityscapes):
    ...
    def __init__(self,
        ...
        self.in_ch = in_ch
        image_list_filepath = os.path.join(self.list_path, self.contrast, self.split +'_' + self.contrast + ".txt")
        if not os.path.exists(image_list_filepath):        raise Warning("split must be train")
        self.images = [id.strip() for id in open(image_list_filepath)]
    ...
    def __getitem__(self, item):
        ...
        slice_id = image_path.split('_')[-1].split('.')[0]
        ttt=  '_'+slice_id+'.nii.gz'
        depths_ = int(self.in_ch-1//2)
        
        
        slice_id_half_min  = '_' + str(int(slice_id)-depths_)  + '.nii.gz'
        slice_id_half_max  = '_' + str(int(slice_id)+depths_)  + '.nii.gz'

        image_path_half_min  = image_path.replace(ttt, slice_id_half_min)
        image_path_half_max  = image_path.replace(ttt, slice_id_half_max)
        
        
        slice_id_full_min = '_' + str(int(slice_id)-int(self.in_ch))  + '.nii.gz'
        slice_id_full_max = '_' + str(int(slice_id)+int(self.in_ch))  + '.nii.gz'

        image_path_full_min  = image_path.replace(ttt, slice_id_full_min)
        image_path_full_max  = image_path.replace(ttt, slice_id_full_max)


        if   os.path.exists(image_path_half_min) and os.path.exists(image_path_half_max) :
            slice_id_new = str(int(slice_id))
            
        elif not os.path.exists(image_path_half_min):
            if os.path.exists(image_path_full_max) :
                slice_id_new = str(int(slice_id))+depths_
            else : 
                slice_id_new = str(depths_)
            
        elif not os.path.exists(image_path_half_max):
            if os.path.exists(image_path_full_min) :
                slice_id_new = str(int(slice_id))-depths_
            else : 
                slice_id_new = str(depths_)
        ...
        image_center = medio.load(image_path)
        
        print(image_center.shape, 'image_center_SHAEP')
        print(image.shape, 'imageSHAEP')
        
        image = np.zeros((self.in_ch, image_center.shape))
...
class CrossMODA_25d_Nd(Cityscapes):
    ...
    def __init__(self,
                 list_path='/home/compu/HG/CrossModa/DRANet-master/DRANet-master/data_list/CrossMODA_2d/',
                 contrast='t1',
                 split='train',
                 crop_size=(256, 256),
                 train=True,
