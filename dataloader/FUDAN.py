...
label_colours_FUDAN = list(map(tuple, label_colours_FUDAN))

def decode_labels_FUDAN(imgs, mask, num_images=1, num_classes=3):
        ...
        for j_, j in enumerate(mask[i, :, :]):
            for k_, k in enumerate(j):
                if k < num_classes and k> 0:
                    img[int(j_), int(k_)] = label_colours_FUDAN[int(k)]
...
class FUDAN_3d(Cityscapes):
        
    def __init__(self,
                 list_path='/home/compu/HG/CrossModa/DRANet-master/DRANet-master/data_list/FUDAN/',
                 contrast='t1',
                 split='train',
                 crop_size=(256, 256),
                 train=True,
...
class FUDAN_25d_Nd(Cityscapes):
    ...
    def __getitem__(self, item):
        ...
        image_center, _ = medio.load(image_path)
        
        image = np.zeros(( image_center.shape[0], image_center.shape[1],self.in_ch ))
        
        
        for k in range(self.in_ch):
            
            image_d , _ = medio.load(image_path)
            image[:,:,k] = image_d[:,:,0]
        ...
        gt_image_center, _ =medio.load(gt_image_path)

        gt_image = np.zeros(( gt_image_center.shape[0], gt_image_center.shape[1],self.in_ch ))
        
        for k in range(self.in_ch):
            
            gt_image_d , _ = medio.load(gt_image_path)
            gt_image[:,:,k] = gt_image_d[:,:,0]
